...
class ControllerContext(val zkUtils: ZkUtils) {
  ...
  // TopicPartition -> 分配给这个Partition的Replica集合
  var partitionReplicaAssignment: mutable.Map[TopicAndPartition, Seq[Int]] = mutable.Map.empty
  // TopicPartition -> 分配给这个Partition的leader和ISR列表
...
class KafkaController(val config: KafkaConfig, zkUtils: ZkUtils, time: Time, metrics: Metrics, threadNamePrefix: Option[String] = None) extends Logging with KafkaMetricsGroup {
  ...
  // visible for testing
  // 本质上是个支持shudown的SingleThreadPoll
   ...
   *
    * 在broker成功注册为controller之后, 回调此方法, 做了以下工作:
    * 1. 注册controller任期(epoch)状态监听(实际上在调用此方法前已经注册, 本方法未作此操作)
    * 2. 递增任期序号
    * 3. 初始化controller上下文(context)对象(缓存了当前的topic列表, 存活broker列表, 所有partition的leader节点)
    * 4. 开启ControllerChannelManager, 负责发送controller发送给其他broker的请求
    * 5. 开启replica state machine, 负责管理replica的状态切换
    * 6. 开启partition state machine, 负责管理partition的状态切换
   */
  def onControllerFailover() {
    info("Broker %d starting become controller state transition".format(config.brokerId))
    // 读取zk中已有的controller epoch/version信息, 写入controllerContext
    readControllerEpochFromZookeeper()
    // +1更新epoch
    incrementControllerEpoch()

    // before reading source of truth from zookeeper, register the listeners to get broker/topic callbacks
    // 注册对几个关键zk节点的监听, 注意监听放在直接读取zk之前, 这样做可以保证并发更新不会导致脏数据
    registerPartitionReassignmentListener() // 分区副本迁移监听
    registerIsrChangeNotificationListener() // ISR列表变动监听
    registerPreferredReplicaElectionListener() // leader最优分布选举监听(用于控制leader均匀分布在所有broker上)
    registerTopicChangeListener() // topic变更监听
    registerTopicDeletionListener() // topic删除监听
    registerBrokerChangeListener() // broker上下线监听

    // 读取zk信息 并 更新controllerContext, 并启动controllerChannelManager(负责处理从controller发送消息到broker)
    ...
    //
    // sendUpdateMetadataRequest必须在下面的两个状态机启动前执行. 因为启动状态机之后会发送LeaderAndIsrRequests给broker处理, broker依赖
    // 于集群的元数据来校验和处理LeaderAndIsrRequests请求
    sendUpdateMetadataRequest(controllerContext.liveOrShuttingDownBrokerIds.toSeq) // call by onControllerFailover
  ...
  def onBrokerStartup(newBrokers: Seq[Int]) {
    ...
    replicaStateMachine.handleStateChanges(allReplicasOnNewBrokers, OnlineReplica) // call on onBrokerStartup
  ...
  def onBrokerFailure(deadBrokers: Seq[Int]) {
    ...
    replicaStateMachine.handleStateChanges(activeReplicasOnDeadBrokers, OfflineReplica) // call on onBrokerFailure
  ...
  def onNewPartitionCreation(newPartitions: Set[TopicAndPartition]) {
    ...
    partitionStateMachine.handleStateChanges(newPartitions, NewPartition) // call on onNewPartitionCreation
    replicaStateMachine.handleStateChanges(controllerContext.replicasForPartition(newPartitions), NewReplica) // call on onNewPartitionCreation
    partitionStateMachine.handleStateChanges(newPartitions, OnlinePartition, offlinePartitionSelector) // call on onNewPartitionCreation
    replicaStateMachine.handleStateChanges(controllerContext.replicasForPartition(newPartitions), OnlineReplica) // call on onNewPartitionCreation
  }
  ...
  def onPartitionReassignment(topicAndPartition: TopicAndPartition, reassignedPartitionContext: ReassignedPartitionsContext) {
    ...
    if (!areReplicasInIsr(topicAndPartition.topic, topicAndPartition.partition, reassignedReplicas)) {
      ...
      reassignedReplicas.foreach { replica =>
        replicaStateMachine.handleStateChanges(Set(new PartitionAndReplica(topicAndPartition.topic, topicAndPartition.partition, // call on onPartitionReassignment
  ...
  def incrementControllerEpoch() = {
    try {
      ...
      // 按照version为条件进行CMS更新, 成功时返回新的epoch和version
  ...
  private def initializeControllerContext() {
    ...
    // 获取存活broker列表. zk路径:ls /brokers/ids
    // 对于当前broker(kafkaServer)启动步骤来到本方法时, 仍未向zk注册成为alive broker, 需要待KafkaController等
    // 组件启动之后, 才通过KafkaHealthcheck注册成为alive broker. 换句话说, 如果当前进程是整个集群的第一个启动的broker, 那么跑到这
    // 里liveBrokers的数量是0
    controllerContext.liveBrokers = zkUtils.getAllBrokersInCluster().toSet
    // 获取所有topic列表, zk路径: ls /brokers/topics
    controllerContext.allTopics = zkUtils.getAllTopics().toSet
    // 获取当前集群内所有TopicPartition 在 replica中的分布情况, (TP -> []ReplicaId), zk路径:get /brokers/topics/<topicName>
    ...
    updateLeaderAndIsrCache() // 从ZK恢复所有partition的leader信息 和 ISR列表
    // start the channel manager
    startChannelManager() // 启动channelManager, 它负责把controller的请求发送给其他broker
  ...
  private def initializePartitionReassignment() {
    ...
    val partitionsBeingReassigned = zkUtils.getPartitionsBeingReassigned() // 分区迁移信息
    // check if they are already completed or topic was deleted
  ...
  // 基于zk持久节点保存的PartitionLeader和ISR列表信息, 更新controllerContext的缓存信息
  ...
  private def stopOldReplicasOfReassignedPartition(topicAndPartition: TopicAndPartition,
    ...
    replicaStateMachine.handleStateChanges(replicasToBeDeleted, OfflineReplica) // call on stopOldReplicasOfReassignedPartition
    // send stop replica command to the old replicas
    replicaStateMachine.handleStateChanges(replicasToBeDeleted, ReplicaDeletionStarted) // call on stopOldReplicasOfReassignedPartition
    // TODO: Eventually partition reassignment could use a callback that does retries if deletion failed
    replicaStateMachine.handleStateChanges(replicasToBeDeleted, ReplicaDeletionSuccessful) // call on stopOldReplicasOfReassignedPartition
    replicaStateMachine.handleStateChanges(replicasToBeDeleted, NonExistentReplica) // call on stopOldReplicasOfReassignedPartition
  }
  ...
  private def startNewReplicasForReassignedPartition(topicAndPartition: TopicAndPartition,
    ...
    newReplicas.foreach { replica =>
      replicaStateMachine.handleStateChanges(Set(new PartitionAndReplica(topicAndPartition.topic, topicAndPartition.partition, replica)), NewReplica) // call on  startNewReplicasForReassignedPartition
    }
  }
    ...
    updateLeaderEpoch(topicAndPartition.topic, topicAndPartition.partition) match {
      case Some(updatedLeaderIsrAndControllerEpoch) =>
        try {
          ...
          brokerRequestBatch.addLeaderAndIsrRequestForBrokers(replicasToReceiveRequest, topicAndPartition.topic, // call on updateLeaderEpochAndSendRequest
        ...
        controllerContext.partitionLeadershipInfo.get(topicAndPartition).foreach { currLeaderIsrAndControllerEpoch =>
          if (replicationFactor > 1) {
            if (currLeaderIsrAndControllerEpoch.leaderAndIsr.leader == id) {
              ...
              replicaStateMachine.handleStateChanges(Set(PartitionAndReplica(topicAndPartition.topic, // call on doControlledShutdown
  ...
  def elect(): Unit = {
    ...
    try {
      ...
      zkCheckedEphemeral.create() // 在zk注册失败会抛出相应异常
      info(config.brokerId + " successfully elected as the controller")
      activeControllerId = config.brokerId // 当前节点成功leader
      onControllerFailover()
    } catch {
      case _: ZkNodeExistsException => // nodeExist, 被其他节点注册
        // If someone else has written the path, then
        // 再次校验当前是否存在Controller节点
        activeControllerId = getControllerID
